...
function formatUnlockedAt(value: string | null) {
  ...
  const matched = value.match(/^(\d{4})-(\d{2})-(\d{2})/);
  return matched
    ? `${matched[1]}.${matched[2]}.${matched[3]}获得`
    : `${value}获得`;
}

function mapAchievement(item: AchievementItem): AchievementCard {
  const image =
    (!item.isUnlocked && LOCKED_ICON_ASSETS[item.iconKey]) ||
    ICON_ASSETS[item.iconKey] ||
    ICON_ASSETS["first-record"];
  const progressText =
  ...
  return {
    ...
    lockedNote:
      item.ruleType === "manual"
        ? item.description
        : `${progressText} · ${item.description}`,
    dateText: item.isUnlocked
      ? formatUnlockedAt(item.unlockedAt)
      : `${item.progressPercent}% 完成`,
    image,
    imageClass: "",
...
export async function getAchievementPageData(): Promise<AchievementPageData> {
  ...
  return {
    ...
    earnedCount: Number(
      data.earnedCount || cards.filter((item) => item.isUnlocked).length,
    ),
